emptylist=[]
# print(emptylist) 
# lst3=[[1,2],[3,4]]  # lists of list
# lst4=[1,'eadd',34,4.45]  # list of different datatypes 
# # print(lst4)
# # print(lst3)
# # print(len(lst3))
# # print(len(lst4))  #len() nested elements ke andar ke count nahi karta — sirf top-level elements count karta hai.
# # lst3.append("dffg")
# # print(lst3)
# # lst4.append(45)  # append kuch rerurn nhi krta islye print(list.append(44)) likhne me none aata hai
# # print(lst4)
# # lst4.insert(2,"astha")
# # print(lst4)
# lst3.remove([3,4]) # starting se dhoodna start kr deta hai
# print(lst3)
# lst3.append(lst4)
# print(lst3)
# lst3.extend(lst4)
# print(lst3)
# del lst3[1]
# print(lst3)
# numbers = [10, 20, 30, 40]
# # removed = numbers.pop(1)   # index 1 pe element 20 hai
# # print(numbers)
# # print("Removed:", removed)
# numbers.remove(20)
# print(numbers)

#keyword 'in' is used  to test 

# lst=['one','two','three']
# if 'one' in lst:
#     print("AI")
# if 'ornrf' not in lst:
#     print("ml")
# lst.reverse()
# print(lst)   
# num=[34,54,22,45,32,34]
# sortee=sorted(num)
# print(sortee)  # print in ascending order
# sorted_lst=sorted(num,reverse=True)  # descending order
# print(sorted_lst)

# astha=[23,45,"addf",56,66]
# sort=sorted(astha)
# print(astha.sort)


# lst=[1,2,3,4,5,55,43] 
# abc=lst
# abc.append(6)
# sort=sorted(lst)
# print(sort)


#sort
# let=[2,34,5,6,7,8]
# let.sort()
# print(let)


# # having multiple references
# lst=[1,2,4,5]
# sbc=lst
# sbc.append(6)
# print(lst)

lst=["mango","apple","orange","banana","cherry"]
lst.reverse()

# lst.toUpp
# for i in lst:
#     print(i.upper())
#     print(i.lower())

# print(lst[-1]) #end vala print hoga
# print(lst[-2])  # secondlast
# for i in lst:
#     print(i)
# abc=len(lst)
# print(abc)
# lst.append("pineapple")
# for it in lst:
#     print("i like ",it)
# lst.insert(1,"kiwi")   #insert at second index so we will use insert function 
# print(lst)  
# lst.remove("pineapple")
# print(lst)
# lst.pop()
# print(lst)
# li=[1,2]
# li1=[3,4]
# c=li+li1
# print(c)

# if "mango" in lst:
#     print("yes")
# else:
#     print("no")
# numbers = [10, 5,5, 30, 2, 8]

# print(min(numbers))   # Output: 2   (sabse chhota number)
# print(max(numbers))   # Output: 30  (sabse bada number)

# # count=0

#   # inbuilt function

# print(numbers.count(5))
# for i in num:
#     if(i==num[0]):



# sum=0
# average=0

# for i in num:
#     sum+=i
    
# print(sum/len(num)) 
a=['x','y','z','y']
for i in a:
    if i==("y"):
        print(i)

print(a.index("y")) #direct




1




